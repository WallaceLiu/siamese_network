...
class SiameseNetwork:
    ...
    def __init__(self, sequence_length, vocab_size, embedding_size, filter_sizes, num_filters, output_embedding_size,
                 dropout_keep_prob, embeddings_lookup, l2_reg_lambda=0.0):
    ...
    def siamese_net(self, input_x, reuse=False):
        ...
        with tf.variable_scope("embedding", reuse=reuse):
            self.W = tf.Variable(self.embeddings_lookup, expected_shape=[self.vocab_size, self.embedding_size], name="W")
    ...
    def contrastive_loss(self, model1, model2, y, margin):
        with tf.name_scope("contrastive-loss"):
            ...
            return tf.reduce_mean(
                y * tf.square(distance) + (1 - y) * tf.square(tf.maximum((margin - distance), 0))) / 2 + 1e-9
